...
class DCCurrentSource(BaseComponent):
    ...
    def _init_connections(self):
        ...
        # Define connection points relative to component position
        self.connections = {
            'pos': (0, 1),   # Positive terminal (top, current flows out)
            'neg': (0, -1)   # Negative terminal (bottom, current flows in)
        }
...
class Diode(BaseComponent):
    ...
    def calculate(self, simulator, time_step):
        ...
        max_current = self.get_property('max_current', 1.0)
        reverse_leakage = self.get_property('reverse_leakage', 1e-6)  # Leakage current in reverse bias
        ...
        if voltage_drop > forward_voltage:
            ...
            # Use the Shockley diode equation: I = Is * (e^(V/Vt) - 1)
...
class LED(Diode):
    ...
    def calculate(self, simulator, time_step):
        ...
        Returns:
            Dictionary of updated state values
        """
        # Get the base diode calculations
        state_updates = super().calculate(simulator, time_step)
        
        # Calculate brightness based on current
        current = state_updates['currents']['anode']
        max_current = self.get_property('max_current', 0.02)
        
        # Brightness is proportional to current, normalized to max_current
        if current > 0:
            brightness = min(1.0, current / max_current)
        else:
            brightness = 0.0
        
        # Add LED-specific state updates
        state_updates['brightness'] = brightness
        state_updates['color'] = self.get_property('color', 'red')
...
class BJT(Transistor):
    ...
    def _init_connections(self):
        ...
        # Define connection points relative to component position
        self.connections = {
            'collector': (0, 1),     # Collector (top)
            'base': (-1, 0),         # Base (left)
            'emitter': (0, -1)       # Emitter (bottom)
        }
