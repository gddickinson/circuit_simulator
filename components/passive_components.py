...
class Ground(BaseComponent):
    ...
    def calculate(self, simulator, time_step):
        ...
        # Calculate total current flowing into ground from all connected components
        current = 0.0
        if node_gnd:
            for component, conn_name in node_gnd.get_connected_components():
                if component.id != self.id:  # Skip self
                    # Get current flowing into this connection point
