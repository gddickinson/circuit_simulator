...
class BaseComponent(ABC):
    ...
    def __init__(self, component_id=None, position=(0, 0), rotation=0, properties=None):
        ...
        self.connected_to = {}  # Dictionary of connected components {connection_name: component_id}
    ...
    def connect(self, connection_name, other_component, other_connection):
        ...
        Args:
            connection_name: Name of the connection point on this component
            other_component: The component to connect to
            other_connection: Name of the connection point on the other component
        ...
        connection_key = f"{connection_name}"
        self.connected_to[connection_key] = (other_component.id, other_connection)
        
        # Store the reverse connection
        other_key = f"{other_connection}"
        other_component.connected_to[other_key] = (self.id, connection_name)
        
        logger.debug(f"Connected {self} {connection_name} to {other_component} {other_connection}")
        return True
    ...
    def disconnect(self, connection_name):
        ...
        """
        connection_key = f"{connection_name}"
        if connection_key not in self.connected_to:
            logger.warning(f"No connection at {connection_name} to disconnect")
            return False
        
        # Get the connected component
        other_id, other_connection = self.connected_to[connection_key]
        
        # Remove the connection
        del self.connected_to[connection_key]
        
        logger.debug(f"Disconnected {self} {connection_name}")
        return True
    ...
    def get_connected_components(self):
        ...
        """
        connections = []
        for connection_name, (other_id, other_connection) in self.connected_to.items():
            connections.append((other_id, connection_name, other_connection))
        return connections
    
    def is_connected(self, connection_name):
        """Check if the given connection is connected.
        ...
        """
        connection_key = f"{connection_name}"
        return connection_key in self.connected_to
